...
export const getSongAsync = async (query: string) => {
  try {
    const options = {
      ...
      headers: {
        "X-RapidAPI-Key": "80b8263e07mshee0dc6a97580ba5p1a96ecjsn0048d359b194",
...
export const createPlayList =
  (data: SinglePlayListProps): AppThunk =>
  async (dispatch) => {
...
export const addSong =
  (data: AddsongProps): AppThunk =>
  async (dispatch) => {
    const allPlaylist = store.getState().playlists.playlists;
    let targetPlaylist = allPlaylist.find(
      (playlist) => playlist.id === data?.playListId
    ) as SinglePlayListProps;
    const targetPlaylistIndex = allPlaylist.findIndex(
      (playlist) => playlist.id === data?.playListId
    );
    ...
    const updatedPlaylist = [
      ...allPlaylist.slice(0, targetPlaylistIndex),
      updatedTargetPlaylist,
      ...allPlaylist.slice(targetPlaylistIndex + 1),
    ];
...
export const makeFavourite =
  (data: MakeFavouriteProps): AppThunk =>
  async (dispatch) => {
    const allPlaylist = store.getState().playlists.playlists;
    ...
    const targetSong = targetPlaylist.songs.find(
      (song) => song.id === data.songId
    );
    const targetSongIndex = targetPlaylist.songs.findIndex(
      (song) => song.id === data.songId
    );
    const updatedSong = {
      ...targetSong,
    ...
    const updatedSongList = [
      ...targetPlaylist.songs.slice(0, targetSongIndex),
      updatedSong,
      ...targetPlaylist.songs.slice(targetSongIndex + 1),
    ];
    ...
    const targetFavouriteIndex = allFavourite.findIndex(
      (favourite) => favourite.id === data.songId
    );
    const updatedFavourite = [
      ...allFavourite.slice(0, targetFavouriteIndex),
      updatedSong,
      ...allFavourite.slice(targetFavouriteIndex + 1),
    ];
